...
\item \bibentry{LKB20} \ \\
Excellent intro on problem of convection and net mass transport. See references.\\
HYMACS not actually implemented in ICON yet. This is just about the necessary ``adaptions to the dynamical core'' which is a ``revised numerical filter'' with ``compatibility to local mass sources''.

\item Revcon proposal: \\
...
\item \bibentry{BM01} \ \\
``study using Mesoscale
Compressible Community model integrations at 2-, 6-, 18-, and 50-km resolution
With convection parameterization:'' \\
``precipitation and internal structures associated with the squall line are thus not well represented at this resolution''

\item \bibentry{MVS+18}
\begin{quote}
Convection schemes are not required for reasonable climatological precipitation. However, they are essential for reasonable daily precipitation and constraining extreme daily precipitation that otherwise develops. Systematic effects on lapse rate and humidity are likewise modest compared with the intermodel spread. Without parameterized convection Kelvin waves are more realistic.
\end{quote}

\item \bibentry{MS08} \ \\
includes idealised model of a squall line

\item \bibentry{HL21} \ \\ Errors at 2km without parameterisation possibly due to downscaled initial conditions (too little CIN). \\
Compensating errors

\item \bibentry{MPS21}

\item \bibentry{WDL21} \ \\
Used 300m London model as truth and compared with lower resolution convection permitting model. \\
``significant biases and vertical correlations exist that should be accounted for to give maximum observation impact in data assimilation and for fairness in model
verification and validation.''

\item \bibentry{BRG+15} \ \\
Comparing with and without convection parameterisation at high resolution. \\
sea-breeze circulations is key for convection initiation over Maritime Continent.\\
``the simulation with parameterized convection is able to reproduce the key rain-forming sea-breeze circulation, the parameterization is not able to respond realistically to the circulation. A feedback of errors also occurs: the convective parameterization causes rain to fall in the early morning, which cools and wets the boundary layer, reducing the land–sea temperature contrast and weakening the sea breeze.''
...
\item \bibentry{WPS10} \ \\
Moist RB convection but BCs don't seem to be fully defined.
...
\item \bibentry{SG94} \ \\
Description of a preliminary version of the Met Office LEM with very simple microphysics and LES. Description of the calculation of the Richarson number is strange.

\item \bibentry{GLC02} \ \\
Part I desripes the joint pdf model - bi Gaussians for sub-grid-scale variablility.\\
1st, 2nd and 3rd order moments of the whole grid box are used to estimate two means and SD of the bi-Gaussians. \\
Differs from \cite{LR01} in that it uses a double Gaussian as the joint PDF.
\item \bibentry{GLC02b} \ \\
Part II compares a single column model with observations

\item \bibentry{LG05} \ \\ CLUBB beginnings but CLUBB not mentioned.\\
``closures for several third- and fourth-order moments. To derive the closures, the
moments are assumed to be consistent with a particular PDF family, namely, a mixture of two trivariate
Gaussians.'' \\
``low-order moments are
extracted from the LES and treated as givens that are input into the closures.''

\item \bibentry{LSW+12} \ \\ First paper about CLUBB \\
``Although PDF-based parameterizations are more complex and computationally expensive than
many other parameterizations, in principle PDF parameterizations have several advantages. For instance,
they ensure consistency of liquid (cloud) water and cloud fraction; they avoid using separate parameteriza-
tions for different cloud types such as cumulus and stratocumulus; and they have an appropriate formulation
in the ``terra incognita'' in which updrafts are marginally resolved.''\\
Comparisons of SAM (System for Atmospheric Modeling) with and without CLUBB.
...
\item \bibentry{LR01} \ \\
``new kind of model that combines HOC and MFC'' (MFC - mass flux closure)\\
``assumed joint probability distribution for the variables of interest, and the equations typically used in HOC models can be derived by integrating over the distribution''
``Assumed-Distribution Higher-Order Closure (ADHOC)''.\\

``For example, the convergence of air as a downdraft approaches the
surface creates high pressure, which acts to slow the air
down and ‘‘splash’’ it out to the side. Conversely, up-
draft air near the surface creates a low pressure wake
behind it. In current mass-flux models, the pressure
terms are most often entirely neglected (although there
are some exceptions, e.g., Wu and Yanai 1994).''
...
Needs high resolution across boundary layer top

Prognose $\overline{w^\prime w^\prime}$ and  $\overline{w^\prime w^\prime w^\prime}$ diagnose $\sigma$ and $M_c=\rho \sigma w_{up}$ from them.

\item \bibentry{WY94} \ \\
``A cumulus momentum prameterization including the convective-scale pressure gradient force is formulated. The pressure gradient force is related to teh vertical wind shear, cloud mass flux and orientation of organised convection. The parameterization is capable of reproducing both the upgradient and downgradient transports of horizontal momentum, provided the mode of organisation of cumulus is known.''
...
\item Soares, P. M. M., P. M. A. Miranda, A. P. Siebesma, and J. Teix-
eira, 2004: An eddy-diffusivity/mass-flux parameterization
for dry and shallow cumulus convection. Quart. J. Roy. Me-
teor. Soc., 130, 3365–3384.

\item \bibentry{LCH+20} \ \\
Single column model compared to LES
...
\item \bibentry{AJ11} \ \\
\begin{quote}
ROUTE I unifies the cumulus parameterization in
...
scale moist convection in such a way that it vanishes when the resolution is sufficiently high.
\end{quote}
\begin{quote}
ROUTE II for the unification follows the MMF approach based on a coupled GCM/CRM,
originally known as the ``super-parameterization''.
\end{quote}
\begin{quote}
...
portion of the GCM grid cell.
\end{quote}

For Route I, $\sigma$ is calulated from the difference between resolved and cloud variables. The eddy transport does not feed back onto the continuity equation. Not implemented.

\begin{quote}
While the performance of the proto-
...
(ii) The CRM must a priori choose a particular direction for
